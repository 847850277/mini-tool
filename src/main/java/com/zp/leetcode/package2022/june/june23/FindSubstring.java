package com.zp.leetcode.package2022.june.june23;
...
public class FindSubstring {
    ...
    public List<Integer> findSubstring1(String s, String[] words) {
        ...
        for (int i = 0; i <= length - total; i++) {
            ...
            find.putAll(map);
            while (left < right) {
                String subStr = s.substring(left, left + size);
                if (find.getOrDefault(subStr, 0) > 0) {
                    find.put(subStr, find.getOrDefault(subStr, 0) - 1);
                    count--;
                    left += size;
                } else {
                    /**
                     *  如果窗口不包含当前子串或者窗口包含当前子串，但窗口中当前子串太多，
                     *  说明当前窗口无效
                     */
                    break;
                }
    ...
    public List<Integer> findSubstring(String s, String[] words) {
        ...
        int subLength = 0;
        int length = s.length();
        int keySize = words[0].length();
        Map<String,Integer> keyMap = new HashMap<String,Integer>();
        for (String word : words) {
            subLength += word.length();
            Integer count = keyMap.getOrDefault(word, 0);
            count++;
            keyMap.put(word, count);
        }
        if(subLength > length){
            return result;
        }
        ...
        //滑动窗口
        int slowIndex = 0;
        String subStr = s.substring(slowIndex,subLength);
        //滑动窗口
        while((subStr.length() == subLength) && slowIndex != (length - subLength)){
            if(match(subStr,keyMap,keySize)){
                result.add(slowIndex);
            }
            slowIndex++;
            subStr = s.substring(slowIndex,subLength + slowIndex);
        }
        if(match(subStr,keyMap,keySize)){
            result.add(slowIndex);
        }
     ...
     * @return
     */
    private boolean match(String subStr, Map<String, Integer> keyMap,int keySize) {
        ...
        for (int i = 1; i <= subStr.length(); i++) {
            final String substring = subStr.substring(slowIndex, i);
            if(newMap.containsKey(substring)){
                Integer count = newMap.get(substring);
                if(count == 1){
                    newMap.remove(substring);
                }else{
                    count--;
                    newMap.put(substring,count);
                }
                slowIndex = i;
            }
            final int diff = i - slowIndex;
            if(diff > keySize){
                return false;
            }
        }
